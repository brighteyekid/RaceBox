#!/usr/bin/env python3
"""
F1 2020 UDP Telemetry Simulator - Stable Mode
Sends simulated F1 2020 car telemetry packets at a stable, reliable rate.

Usage:
    python sim_send_f1_stable.py [ESP8266_IP]

Default ESP8266 IP: 172.20.10.14 (adjust for your hotspot)
"""
...
def create_packet_header(packet_id, session_time, frame_id, player_car_index=0):
    """Create F1 2020 packet header (25 bytes)"""
...
def main():
    ...
    print(f"F1 2020 UDP Telemetry Simulator - STABLE MODE")
    print(f"Target ESP8266: {esp8266_ip}:{F1_UDP_PORT}")
    print(f"Sending at 5 Hz (every 200ms) for stability")
        ...
        while True:
            ...
            try:
                ...
                if current_time - last_print_time >= 5.0:
                    print(f"Frame {frame_id:6d}: Speed={speed:6.1f} km/h, Gear={gear}, RPM={rpm:4d} | Sent: {packets_sent}, Errors: {send_errors}")
                    last_print_time = current_time
                    
            except Exception as e:
                send_errors += 1
                if send_errors % 10 == 1:  # Print every 10th error
                    print(f"Send error #{send_errors}: {e}")
            
            frame_id += 1
            
            # Send at 5 Hz (every 200ms) for better stability
            time.sleep(0.2)
            
    except KeyboardInterrupt:
        print(f"\nStopping simulator...")
        print(f"Total packets sent: {packets_sent}")
        print(f"Total errors: {send_errors}")
        print(f"Success rate: {(packets_sent/(packets_sent+send_errors)*100):.1f}%")
